...
const CartaoForm: React.FC<CartaoFormProps> = ({
  ...
  const carrosDisponiveis = queryCarros.data
    ? queryCarros.data.filter(
        (carro) =>
          !queryCartoes.data?.some(
            (cartao) => cartao.idCarro === carro.idCarro
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const cartaoParaSalvar = cartaoEditado
      ? { ...cartao }
      : { ...cartao, senha };
    ...
    <div className="fixed p-3 inset-0 mt-20 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-4xl">
        <h2 className="text-2xl font-bold mb-6">
          {cartaoEditado ? "Editar Cartão" : "Adicionar Cartão"}
        </h2>
        <form
        ...
        >
          ...
          {!cartaoEditado && (
            <>
              ...
              {erroSenha && (
                <p className="text-red-500 text-sm col-span-2">{erroSenha}</p>
              )}
            </>
          )}
